...
local environment = setmetatable({}, {
    __call = function(self, scope, store)
        ...
        local EXIST_PREDICATE = instance:create_metasymbolic_entity("j.t.")
        instance.EXIST_PREDICATE = EXIST_PREDICATE
        instance.EXIST_DECLARATION = {EXIST_PREDICATE}

        make_exist_categorized(instance, "j.t.", "#constraint")
        make_exist(instance, EXIST_PREDICATE)
        return instance
    end
})
    ...
    if head == "realize" or head == "each" then
        ...
        for _, entity in iterate(env:evaluate(argument)) do
            if not env:check_constraints(entity, constraints) then
                ...
                if not mt or not mt.__gilatoft_optional then
                    return false
                end
            end
        end
...
make_exist = function(env, entity, target)
    record_store(env, entity, env.EXIST_PREDICATE, env.EXIST_DECLARATION)
end

make_exist_categorized = function(env, entity, target, ...)
    local EXIST_PREDICATE = env.EXIST_PREDICATE
    record_store(env, entity, EXIST_PREDICATE,
        {EXIST_PREDICATE, {d = {"realize", ...}}})
end
...
function environment:create_metasymbolic_entity(symbol)
    local entity = setmetatable({}, entity_mt)
    entity.name = "symbol: "..symbol
    record_store(self, entity, symbol, {symbol})
    return entity
end

function environment:create_symbolic_entity(symbol)
    local entity = setmetatable({}, entity_mt)
    entity.name = "symbol: "..symbol

    record_store(self, entity, symbol, {symbol})
    make_exist_categorized(self, symbol, "#constraint")
    make_exist(self, entity)

    return entity
end

function environment:find_entity(constraints)
    local store = self.store
    local max_slot_size = self.max_slot_size
    ...
    for i = 1, #constraints do
        ...
        if queue and queue.front ~= 0 then
            for j = queue.rear, queue.front, -1 do
                if j == 0 then j = max_slot_size end

                local entry = queue[i]
                local entity = entry[1]
                local declaration = entry[2]

                if not found_entities[entity]
                    and self:check_constraints(declaration, declaration[3]) then
                    found_entities[entity] = true
                    if self:check_constraints(entity, constraints) then
                        return entity
                    end
                end
            end
...
function environment:find_symbolic_entity(symbol)
    local store = self.store
    local max_slot_size = self.max_slot_size

    local queue = store[symbol]
    if not queue or queue.front == 0 then
        return nil
    end

    return queue[queue.rear][1]
end

function environment:acqure_entity(constraints)
    return self:find_entity(constraints)
        or self:create_entity(constraints)
end

function environment:acqure_symbolic_entity(symbol)
    return self:find_symbolic_entity(symbol)
        or self:create_symbolic_entity(symbol)
end

function environment:apply_constraint(entity, constraint)
    if type(constraint) == "table" then
        local predicate_symbol = constraint[1]
        local predicate = self:acqure_symbolic_entity(predicate_symbol)
        ...
        local predicate = self:acqure_symbolic_entity(constraint)
        record_store(self, entity, predicate, {predicate})
    end
...
local function do_check_constraint(env, entity, predicate_symbol, arguments, constraints)
    local predicate = env:acqure_symbolic_entity(predicate_symbol)
...
function environment:check_constraint(entity, constraint)
    ...
    if arguments and arguments.n then
        return self:assert(predicate_symbol, arguments, subconstraints)
    end

    if type(predicate_symbol) == "table" then
        local t = {}
        for i = 1, #predicate do
            if not do_check_constraint(
                self, entity, predicate_symbol[i], arguments, subconstraints) then
                t[#t+1] = declaration
            end
        end
        return unpack(t)
        ...
        return do_check_constraint(
            self, entity, predicate_symbol, arguments, subconstraints)
    end
end
    ...
    return function(state, index)
        ...
        while true do
            for i = curr_entry_index, curr_queue.front, -1 do
                ...
                local entry = curr_queue[i]
                local entity = entry[1]
                local declaration = entry[2]

                if not found_entities[entity]
                    and env:check_constraints(declaration, declaration[3]) then
                    found_entities[entity] = true
...
local select_iter = make_entity_iterator(function(env, entity, curr_constraint, constraints)
    return env:check_constraints(entity, constraints)
end)
...
function environment:declare(predicate_symbol, arguments, constraints)
    local predicate = self:acqure_symbolic_entity(predicate_symbol)
    local declarations = self.declarations
    local declaration = setmetatable({
        predicate, arguments, constraints,
        name = "declaration: "..predicate_symbol
    }, entity_mt)
...
function environment:assert(predicate_symbol, arguments, constraints)
    local predicate = self:acqure_symbolic_entity(predicate_symbol)
...
function environment:query(predicate_symbol, arguments, constraints)
    local predicate = self:acqure_symbolic_entity(predicate_symbol)
    local entry = self.declarations[predicate]
    if not entry then return nil end
